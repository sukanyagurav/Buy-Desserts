...
const Modal = ({isOpen,closeModal}) => {
  ...
  return createPortal(
    <div>
        ...
        <motion.dialog  initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        exit={{ opacity: 0, y: 30 }} open={isOpen} className='top-[10%] mx-auto w-[100%] fixed  max-w-[600px] z-50 rounded-lg  bg-[--rose-50] p-8 px-5'>
            <img 
              src="/images/icon-order-confirmed.svg" alt="" className='w-10 h-10' />
            <h2 className=' text-2xl my-2 mb-0 text-[--rose-900] font-bold'>Order Confirmed</h2>
            <p className='text-[--rose-500]'>We hope you enjoy your food!</p>
           <ul  className='p-5 py-3 bg-[--rose-100] my-5 rounded-md overflow-y-scroll h-[300px] flex flex-col  cart_list' >
            {cart.map(cartItems=>(
             <li key={cartItems.name} 
                className=' py-4 border-[--rose-300] border-b-[0.3px] items-center flex gap-4 '>
               ...
               alt={cartItems.name} className='w-20 h-20 rounded-lg object-contain  drop-shadow-lg' />
                ...
                whileInView={{ clipPath: 'polygon(0 0, 100% 0, 100% 100%, 0 100%)' }}
              viewport={{ once: true }}
              transition={{ duration: 0.5 }}   className='text-[--rose-500] ml-auto font-semibold'>${cartItems.totalPrice.toFixed(2)}</motion.span>
           
          </li>
        ))}
        <li   className='flex justify-between items-center my-4 pt-2 mt-auto' >
            <h6 className='text-[--rose-900]  text-sm mb-1 font-semibold'>Order Total</h6>
            <h3 className='text-[--rose-900] font-bold text-2xl'>${cartTotal.toFixed(2)}</h3>
        </li>
      </ul>
      <button onClick={handleSubmit} className='bg-[--red] p-3 rounded-full w-full mt-8  text-white' >
        Start New Order
      </button>
        </motion.dialog> 
